...
const API_URL = 'https://api.themoviedb.org/3/discover/movie?api_key=25afca5b22e187755c2665b7a304437e&language=en-US&include_adult=false&include_video=false&page=1&with_watch_monetization_types=flatrate&page=1&with_genres=9648';
...
window.addEventListener('scroll', () => {
  ...
  if (scrollTop + clientHeight >= scrollHeight && !fetching) {
    fetching = true;
    const url = `https://api.themoviedb.org/3/discover/movie?api_key=25afca5b22e187755c2665b7a304437e&language=en-US&include_adult=false&include_video=false&page=${currentPage + 1}&with_genres=9648`;
    fetch(url)
      .then(res => res.json())
      .then(data => {
        showMovies(data.results);
